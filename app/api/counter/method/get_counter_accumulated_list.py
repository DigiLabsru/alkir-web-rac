...
def get_counter_accumulated_list(req: ResourceConsumptionCounterValueInfo):
    try:
        rac = RasInterface(req=req)
        SimpleDateFormat = jpype.JClass('java.text.SimpleDateFormat')
        sdf = SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
        rac_data = rac.java_get_resource_consumption_counter_accumulated_values(counter_name=req.counter_name, object_name=req.object_name)
        rac.close()
